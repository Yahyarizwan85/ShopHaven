...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if Customer.objects.filter(email=email).exists():
            messages.error(request, "Email alrea registered.")
            return render(request, 'Signup.html', {'value': value})
        else:
...
def login(request):
    if request.method == 'GET':
        ...
        email = request.POST.get('email')
        password = request.POST.get('password')
        user_customer = Customer.get_customer_by_email(email)  # Get customer by email
        ...
        if user_customer:
            ...
            if check_password(password, user_customer.password):
                ...
                messages.error(request, "Invalid Email or Password")  # Wrong password
        else:
            messages.error(request, "Email or Password is invalid.")  # User does not exist
