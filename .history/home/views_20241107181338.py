...
#         email = request.POST.get('email')
#         password = request.POST.get('password')
#         user_customer = Customer.get_customer_by_email(email)
#         print(user_customer)
...
def login(request):
    if request.method == 'GET':
        ...
        if user_customer:
            ...
            if user_customer.check_password(password):
                # You might want to set session data here or use Django's login function
                request.session['customer_id'] = user_customer.id
                return redirect('home')  # Redirect to the home page or any other page on successful login
            else:
