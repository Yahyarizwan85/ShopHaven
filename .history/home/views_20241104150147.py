...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if not last_name or len(last_name) < 4:
            messages_error(request, "Last name must be at least 4 characters long.")
            return render(request, 'Signup.html')
        
        if not phone or not phone.isdigit() or len(phone) != 12:
            messages_error(request, "Phone number must be exactly 12 digits.")
            return render(request, 'Signup.html')
        
        try:
            validate_email(email)
        except ValidationError:
            messages_error(request, "Please enter a valid email address.")
            return render(request, 'Signup.html')
        
        if not password or len(password) < 8:
            messages_error(request, "Password must be at least 8 characters long.")
            return render(request, 'Signup.html')
        
        # Check if email already exists
        messages_error = None
        if Customer.objects.filter(email=email).exists():
            messages_error(request, "Email already registered.")
        else:
    ...
    return render(request, 'Signup.html', {'error'} : messages_error)
